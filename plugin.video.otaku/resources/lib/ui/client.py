...
from resources.lib.ui.control import log, cache
...
def request(
    ...
    try:
        ...
        if 'User-Agent' in _headers:
            ...
            _headers['User-Agent'] = cache(randommobileagent)
        else:
            _headers['User-Agent'] = cache(randomagent)
        ...
        try:
            ...
            log('Request-Error (%s): %s' % (e.reason, url))
            return ''
            ...
            if response.code == 403 and "cloudflare" in server.lower():
                ...
                try:
                    ...
                    if 'return' in error:
                        # Give up
                        log('Request-Error (%s): %s' % (response.code, url))
                        return ''
                    else:
                        if not error:
                            return ''
        ...
        log('Request-Error: (%s) => %s' % (str(e), url), 'info')
        return
